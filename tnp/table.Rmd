...
```{r, echo=FALSE, fig.cap="Main performance metrics (DR-learner, no CV)", out.width = '100%', fig.align="center", fig.pos="H"}
knitr::include_graphics("~/Repo/te_vim/tnp/plot_dots_dr_nocv_new_v.png")
```
...
data_long12$Method[which(data_long12$Method == "TMLE")] <-  "TMLE1"
# data_long12 <- data_long12 %>% filter(Method == "TMLE1")

res <- read_hp(filename = 'local_earth_new_', savedate = '2022-10-17')
data_long13 <- proc_df_tbl(res, N_len = 8)
data_long13$Method[which(data_long13$Method == "TMLE")] <-  "TMLE2"
data_long13 <- data_long13 %>% filter(Method == "TMLE2")

res <- read_hp(filename = 'local_earth_new2_', savedate = '2022-10-18')
data_long14 <- proc_df_tbl(res, N_len = 8)
data_long14$Method[which(data_long14$Method == "TMLE")] <-  "TMLE3"
data_long14 <- data_long14 %>% filter(Method == "TMLE3")


res <- read_hp(filename = 'local_hal_nocv_', savedate = '2022-10-21')
data_long15 <- proc_df_tbl(res, N_len = 8)
data_long15$Method[which(data_long15$Method == "TMLE")] <-  "TMLE1"
data_long15 <- data_long15 %>% filter(Method == "TMLE1")



data_long <- rbind(data_long12 %>% mutate(Q_g_model = 'Earth'), 
                   data_long13 %>% mutate(Q_g_model = 'Earth'),
