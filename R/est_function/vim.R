...
run_VIM_Theta <- function(df, 
  ...
  # tmle_vim
  if (tmle_b){
    resTMLE <- TMLE_VIM_b(df_fit, y_l, y_u, max_it)
  }else{
    resTMLE <- TMLE_VIM_a(df_fit, y_l, y_u, max_it)
  }
...
HAL_VIM <- function(data){
  ...
  coef <- theta_s_0
  std_err <- ss
  names(coef) <- names(std_err) <- c("HAL_Theta_s")
...
TMLE_VIM_b <- function(data, y_l, y_u, max_it = 600, lr = 1e-4){
  N <- NROW(data)
  A <- data$A
  Y <- data$Y
...
## old implementation (mixed fluctuation, deprecated Jan 1, 2023)
# TMLE_VIM_a <- function(data, y_l, y_u, max_it = 600, lr = 1e-4){
#   N <- NROW(data)
#   A <- data$A
#   Y <- data$Y
