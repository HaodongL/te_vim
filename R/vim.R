...
#   # sig1, sig2
#   sig1 <- sd(2*(tau_0 - tau_s_0)*((2*A-1)/gn)*(Y - QA_0))
#   sig2 <- sd((tau_0^2 - gamma_s_0) - 2*tau_s_0*(tau_0 - tau_s_0))
#   
#   # eps1,eps2
#   eps1 <- 1e-4
#   eps2 <- 1e-4
...
#     # update gamma_s
#     H_3i <- 1
#     D_3i <- H_3i*(tau_i^2 - gamma_s_i)
#     PnD_3i <- mean(D_3i)
#     
#     eps3_new <- min(c(min(gamma_s_i)-1e-6, eps3, 1-max(gamma_s_i)-1e-6))
#     gamma_s_i <- (gamma_s_i) + eps3_new*sign(PnD_3i)
#     
#     # suppressWarnings({
#     #   logitUpdate <- glm(tau_i^2 ~ 1,
#     #                      offset = qlogis(gamma_s_i),
#     #                      family = "quasibinomial")
#     # })
#     # eps3 <- coef(logitUpdate)
#     # gamma_s_i <- plogis(qlogis(gamma_s_i) + eps3)
#     
#     # update D1 D3-D2, check creteria
#     H1_1i <- 2*(tau_i - tau_s_i)*((2*1-1)/gn)
#     H0_1i <- 2*(tau_i - tau_s_i)*((2*0-1)/gn)
#     HA_1i <- ifelse(A, H1_1i, H0_1i)
#     D_1i <- HA_1i*(Y - QA_i)
...
#     
#     D_3i <- H_3i*(tau_i^2 - gamma_s_i)
#     PnD_23i <- mean(D_3i - D_2i)
#     
#     c1 <- abs(PnD_1i) <= sig1/(sqrt(N)*log(N))
#     # c2 <- abs(PnD_2i) <= sig2/(sqrt(N)*log(N))
#     c2 <- abs(PnD_23i) <= sig2/(sqrt(N)*log(N))
#     
#     if (c1 & c2){
#       break
#     }
