...
fit_tau <- function(df, df_fit, tau_bounds = NULL, dr = TRUE, cv = TRUE){
...
fit_tau_s <- function(df, df_fit, tau_s_bounds = NULL, dr = TRUE, cv = TRUE, ws = NULL){
  ...
  if (dr){
    ...
    task_cate <- sl3::make_sl3_Task(
      data = df_train,
      covariates = setdiff(names(df_train), c('Y', 'A', 'po', ws)),
      outcome = 'po',
      folds = folds
    )
    ...
    task_cate <- sl3::make_sl3_Task(
      data = df_train,
      covariates = setdiff(names(df_train), c('Y', 'A', 'tau', ws)),
      outcome = 'tau',
      folds = folds
    )
...
# fit_tau_s <- function(df, df_fit, cv = TRUE, ws = NULL){
# 
#   tau = df_fit$tau
#   df_train <- cbind(df, tau)
#   folds <- origami::folds_vfold(nrow(df))
...
fit_gamma_s <- function(df, df_fit, gamma_s_bounds = NULL, dr = TRUE, cv = TRUE, ws = NULL){
  
  if (dr){
    po_2 = (df_fit$po)^2
    df_train <- cbind(df, po_2)
    folds <- origami::folds_vfold(nrow(df))
    
    # setup sl3
    task_gamma <- sl3::make_sl3_Task(
      data = df_train,
      covariates = setdiff(names(df_train), c('Y', 'A', 'po_2', ws)),
      outcome = 'po_2',
      folds = folds
    )
    ...
    task_gamma <- sl3::make_sl3_Task(
      data = df_train,
      covariates = setdiff(names(df_train), c('Y', 'A', 'tau_2', ws)),
      outcome = 'tau_2',
      folds = folds
    )
...
# fit_gamma_s <- function(df, df_fit, gamma_s_bounds = NULL, cv = TRUE, ws = NULL){
#   
#   tau_2= (df_fit$tau)^2
#   df_train <- cbind(df, tau_2)
#   folds <- origami::folds_vfold(nrow(df))
